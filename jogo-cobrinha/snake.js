...
class RLSnake {
  ...
  reward(state, action) {
    ...
    let stringifycurrentState = JSON.stringify(state);
    let stringifyFutureState = JSON.stringify(futureState);

    if (stringifycurrentState != stringifyFutureState) {
      if (
        (state[0] == 0 && action == 0) ||
        (state[1] == 0 && action == 1) ||
        (state[1] == 0 && action == 2)
    ...
    let melhorFutureValue = Math.max(
    ...
    let valorAtualizado =
      this.alpha *
        (rewardForState +
          this.gamma * melhorFutureValue -
          this.getQ(state, action)) -
      0.0001;
    this.setQ(state, action, valorAtualizado);
  }
